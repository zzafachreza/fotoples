...
$sql = mysqli_query ($conn, "SELECT * FROM `transaksi_bayar` INNER JOIN `transaksi` ON (`transaksi_bayar`.`inv` = `transaksi`.`inv`) INNER JOIN `media_cetak` ON (`transaksi`.`id_media` = `media_cetak`.`id_media`) INNER JOIN `pelanggan` ON (`transaksi`.`id_pelanggan` = `pelanggan`.`id_pelanggan`) INNER JOIN `kategori_cetak` ON (`media_cetak`.`id_kategori` = `kategori_cetak`.`id_kategori`) WHERE status_transaksi='PROSES' GROUP BY transaksi_bayar.inv ");
 ...
 -->
                        ...
                        <!--       <td><?php echo $tampil['telpon_pelanggan'] ?></td>
                              ...
                              <td><?php echo number_format($tampil['bayar']) ?>
                                ...
                                <form id="formbelum<?php echo $tampil['inv'] ?>" onsubmit="editBayar('<?php echo $tampil['inv'] ?>');return false" style="display: none">
                                        ...
                                        <div class="form-group">
                                         ...
                                         <div class="form-group">
                                          <label>Bayar</label>
                                           <input id="bayarbelum<?php echo $tampil['inv'] ?>" type="" name="bayar" class="form-control" autocomplete="off"  style="width: 400px">              
                                        </div>
                              ...
                              <td> <button data-pelanggan="<?php echo $tampil['id_pelanggan'] ?>" data-id="<?php echo $tampil['inv'] ?>" class="editbelum btn btn-primary" data-toggle="tooltip" data-placement="top" title="edit" ><i class="fa fa-pencil-alt"></i></button>
                                <a target="_BLANK" href="print.php?inv=<?php echo $tampil['inv']  ?>" style="color: #FFF" class="edit btn btn-danger" data-toggle="tooltip" data-placement="top" title="print" ><i class="fa fa-print"></i> Print</a>
                              </td>
                            </tr>
                        ...
                        <script type="text/javascript">
                                    ...
                                    function editBayar(x){
                                      var data =  $("#formbelum"+x).serialize();
                                      // alert(data);
                                       $.ajax({
                                        ...
                                        success:function(data){
                                          // alert(data);
                                           $.ajax({
                                            ...
                                            success:function(data){
                                             location.href='home.php?open=belum'
                                            }
                                          })
                                      ...
                                      $(".editbelum").click(function(e){
                                      e.preventDefault();
                                      var inv = $(this).attr("data-id");
                                      $("#formbelum"+inv).slideToggle();
                                      $("#bayarbelum"+inv).focus();
